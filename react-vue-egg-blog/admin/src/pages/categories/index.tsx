...
function Categories() {
  ...
  async function fetchData(current = 1, pageSize = 20, params = {}) {
    ...
    try {
      const postData = { page: current, pageSize, ...params }; 
  ...
  function onChangeTable(pagination) { // 表格：分页
    const { current, pageSize } = pagination;
    fetchData(current, pageSize, formParams);
  }

  function onSearch(name) { // 表头：搜索框
  ...
  const onHandleSave = async (row) => { // 行数据：保存
  ...
  const onDelete = async (row) => { // 行数据：删除
  ...
  const columns = [
    {
      title: <span style={{fontWeight: 'bold', color: 'red'}}>分类名称(可编辑)</span>,
      dataIndex: 'name',
    ...
    },
    {
      ...
      render: (_, record) => { return record.createTime ? dayjs(record.createTime * 1000).format('YYYY-MM-DD HH:mm:ss') : '-'; },
    },
    {
      title: '修改时间',
      dataIndex: 'updateTime',
      render: (_, record) => { return record.updateTime ? dayjs(record.updateTime * 1000).format('YYYY-MM-DD HH:mm:ss') : '-'; },
    }, 
    {
      ...
      <Card bordered={false}>
        <div className={styles.toolbar}> 
          ...
          <div>
            ...
            <Input.Search style={{ width: 300 }} searchButton placeholder="请输入分类名称" onSearch={onSearch}/>
          </div>
        </div>
        ...
        <Table
          ...
          columns={columns.map((column) => //这里面循环是数据 dataIndex要对应data里面的属性名 如果要渲染多组件 需要使用render()
            column.editable ? { ...column, onCell: () => ({ onHandleSave })} : column
          )}
          data={data}
        ...
        <Modal title={<div> 添加分类 </div>} visible={visible} onOk={onOk} confirmLoading={confirmLoading} onCancel={onCancel}>
