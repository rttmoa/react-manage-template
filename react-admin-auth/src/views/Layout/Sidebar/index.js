            ...
            <SubMenu key={route.path} title={<span><Icon type={route.icon}/><span>{route.name}</span></span>}>
                {children.map( item => {
                    let names = [route.name],
                        paths = [route.path]
                    return !item.children ? (<Menu.Item key={item.path}>
                        <Link to={item.path} onClick={e => { if (pathname !== item.path) history.push(item.path); e.preventDefault(); }}>{item.name}</Link>
                    </Menu.Item>) : createSubMenuComponents(item, pathname, history); // 如果children里面有children继续回调
                })}
            </SubMenu>
    )
}
...
class SideBar extends React.Component {
    ...
    render () {
        ...
        // console.log("breadCrumbs", [breadCrumbs[breadCrumbs.length-1].path])

        return (
            <div id="sidebar_wrapper">
                <Layout>
                    <Sider trigger collapsed={this.props.state.collapsed}>
                        <div className="logo">
