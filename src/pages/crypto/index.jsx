...
// ? AES加密解密、REA加密解密w
const MyCrypto = () => {
  const [aesData, setAesData] = useState() // 大型语言模型 文本
  ...
  useEffect(() => {
    const encryptData = () => {
      setAesKey(CryptoJS.lib.WordArray.random(32).toString()) // set AES密钥：
      ...
      setAesData(aseParameter) // set AES加密明文：
      setAesEncrypt(aesEncrypt) // set AES加密密文：
      const aesDecrypted = AES.decrypt(aesEncrypt, aesKey).toString(enc.Utf8)
      setAesDecryptData(aesDecrypted) // set AES解密后文件：
      ...
      setReaData(reaParameter) // set REA加密明文：
      ...
      setReaDecryptData(uncrypted) // set REA解密后：
    }
  ...
  return (
    <FixTabPanel>
      <section style={{ wordBreak: 'break-all' }}>
        <h3><b>AES加密明文：</b> {aesData}</h3>
        <h4><b>AES密钥：</b>{aesKey}</h4>
        <h3><b>AES加密密文：</b>{aesEncryptData}</h3>
        <h3><b>AES解密后文件：</b>{aesDecryptData}</h3>

        <h3><b>REA加密明文：</b>{reaData}</h3>
        <h4><b>REA加密密文：</b>{reaEncryptData}</h4>
        <h4><b>REA解密后：</b>{reaDecryptData}</h4>
      </section>
    </FixTabPanel>
  )
}
