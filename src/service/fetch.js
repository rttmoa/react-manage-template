...
const handleFetchData = (url, options) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      handleFailedResult({ status: 50000, statusText: 'Time out' }, new Error('TimeoutError: Timeout for Promise'), isShowError)
...
// ? 请求 Nodejs 接口
export const reqFetch = (url, params = { method: 'GET', payload: null, headers: null, isShowError: true, timeout: 20000, controller: null }) => {

  console.log("参数：", params)

  const { method = 'GET', payload = null, headers = null, isShowError = true, timeout = 20000, controller = null } = params;
  ...
  const options = isNoReqBody ? defaultOptions : { ...defaultOptions, body: JSON.stringify(payload) } // "{\"name\":\"中国\",\"age\":22}"

  const suffixPayload = isNoReqBody ? { ...payload } : {} // 如果是 GET 请求，将payload对象转化为地址栏信息

  const signedPayload = suffix(suffixPayload) // 添加后缀： ?timestamp=1703768603219
  ...
  // ? 处理GET、POST等请求和请求地址  &  函数返回 Promise
  return handleFetchData(fetchUrl, options)
}

//  GET 请求时，将payload对象解析成地址栏请求参数
export const getFetch = (url, params) => reqFetch(url, { method: 'GET', ...params })

export const postFetch = (url, params) => reqFetch(url, { method: 'POST', ...params })

export const putFetch = (url, params) => reqFetch(url, { method: 'PUT', ...params })

export const deleteFetch = (url, params) => reqFetch(url, { method: 'DELETE', ...params })

export const patchFetch = (url, params) => reqFetch(url, { method: 'PATCH', ...params })
