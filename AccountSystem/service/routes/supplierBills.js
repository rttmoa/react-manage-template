...
router.route('/').get(function (req, res, next) { // ? 供应商对账列表
                ...
                if (err) {
                    ...
                    if (count > 0) {
                        ...
                        Supplier.find({}, function (err, suppliers) {
                            ...
                            debtSuppliers.map((supplier) => supplier['supplierName'] = supplierMap[supplier['_id']])
                            bills.map((bill) => bill['supplierName'] = supplierMap[bill['supplierId']])


                            console.log("debtStorages", debtStorages)
                            console.log("debtSuppliers", debtSuppliers)
...
router.route('/doClearStorage').post(function (req, res, next) { // !更新供应商库存支付金额
    let currentUser = req.session.userInfo
...
router.route('/doClearBill').post(function (req, res, next) { // !更新供应商账单支付金额
        let currentUser = req.session.userInfo
