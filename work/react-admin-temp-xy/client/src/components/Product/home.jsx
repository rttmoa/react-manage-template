...
class ProductHome extends Component {
  ...
  search = () => {
    setTimeout(() => {
      this.setState(
        { loading: true, productValue: this.productValueNode.state.value },
        () => {
          // setState先设置，回调中执行搜索接口
  ...
  render() {
    const { loading, productList, total, productValue, pageSize, productType } =
      this.state;
    const title = (
      <span>
        <Select
        ...
        <Card
          ...
          extra={
            <Button
              ...
              type="primary"
              onClick={() =>
                this.props.history.push("/product/addUpdate?title=添加")
              }
            >
              添加商品
            </Button>
              ...
              render={(status, product) => {
                return (
                  <span>
                    <Button
                      type="primary"
                      onClick={this.handUpdateStatus(product)}
                    >
                      {status === 1 ? "下架" : "上架"}
                    </Button>
              ...
              render={(category, record) => (
                <Space size="middle">
                  ...
                  <Button
                    type="link"
                    onClick={() =>
                      this.props.history.push("/product/addUpdate?title=修改", {
                        category,
                      })
                    }
                  >
